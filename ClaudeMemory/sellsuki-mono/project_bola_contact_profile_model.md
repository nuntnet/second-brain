...
**Reader/writer surface of the 4 denormalized cols (customer.email/phone/tags/custom_fields) — grep sweep 2026-07-18, now design-doc §13 + cards:** concept cards named the cutover but MISSED concrete sites. Full list: search/filter query (customer_repository/postgres_gorm.go:125-157/226-235 — `WHERE email=?/phone ILIKE/tags LIKE/custom_fields->>` → **whole Contacts endpoint 500s at R3, not blank**) 🔴; outbound_event.go:43-45 FollowerEventPayload emits to external → **silent contract break** 🔴; export_job.go runFollowerExportJob (rewrite) + runContactExportJob (phone join) — runPhoneContactExportJob already correct=reference; route/v1/customer/model.go:129-133 response serialize; registration_form.go:354-368 (write path SEPARATE from UpdateFollower); lon_job.go:933-1200 + pnp.go read c.Phone for by-phone. CLEAN (via cp already): broadcast/auto-push targeting, analytics. **KEY: R3 drop must be gated on a mechanical grep/compile check = zero refs to those columns (can't verify this blast radius by memory) + `bola_contact_dualwrite_divergence_total=0`.**
