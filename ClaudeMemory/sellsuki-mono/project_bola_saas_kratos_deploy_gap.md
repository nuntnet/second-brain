...
**⚠️ PROD OATHKEEPER SYNC LANDMINE:** ArgoCD `ory-oathkeeper-production` is OutOfSync because the live `share/oathkeeper-rules` ConfigMap has **34 access rules managed out-of-band** (the "bridge" mechanism) while git (`ory-helm production/oathkeeper-values.yaml`) has `accessRules: ""`. **Running `argocd sync ory-oathkeeper-production` would OVERWRITE oathkeeper-rules with empty → wipe all 34 rules → break auth for patona/octoplus/sellsuki/file-service prod-wide.** Never sync that app without SRE reconciling the drift first. Oathkeeper rule/CORS changes for prod must go through the out-of-band bridge mechanism, NOT argocd sync.
