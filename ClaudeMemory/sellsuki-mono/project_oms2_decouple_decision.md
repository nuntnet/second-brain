...
**Fulfillment handoff (จาก user 2026-07-11):** `prepare` = ปุ่มของแผนกขาย ส่งมอบงานเข้า **Fulfillment Hub** (เลือกสาขาผู้ fulfill ได้ — hub ของแต่ละสาขาเห็น order ทั้งของร้านตัวเองและที่ส่งมาจากสาขาอื่น) → order = `preparing`; ห้ามข้ามขั้นนี้ และ**ห้ามยุบ step ใน walkthrough ทุกเคส** (user reject การย่อ "เส้นเดียวกับ A2" ใน A5/A8/A11 — ทุกเคสต้องกดเดิน prepare→pack→ship→carrier hook ทีละก้าวจริง); `delivered` มาจาก **carrier hook เท่านั้น** ไม่ใช่ปุ่มเรา; A12 (3PL/FBS): กด "ส่ง order เข้า WMS" = prepare แล้วทุก transition จากนั้นเป็น WMS hook; tracking แบ่ง standard shipping (แพลตฟอร์ม→เรา + label) vs own fleet (เรา→แพลตฟอร์ม) — encode ใน PAT-2240 + artifact v6
