...
metadata:
  ...
  modified: 2026-09-11T04:39:37.743Z
---
...
**🔴 CORRECTION (2026-09-11): schema and config are DIFFERENT permission families, and read and write are different permissions too.** Measured against staging with one identity: `GET /v1/configuration/portal-app-registry` → **200**, `GET /v1/schema/portal-app-registry` → **403**, `PUT /v1/configuration/portal-app-registry` → **403**. So `configsystem.config.view` was granted while `configsystem.schema.view` and `configsystem.config.update` were not. The four config permissions are `sellsuki.configsystem.config.{view,create,update,delete}` and the schema ones `sellsuki.configsystem.schema.{view,create,update}` (`entity@v0.33.0/access_control/permission_list.go:105-114`); `src/use_case/schema.go:21,39,56` uses the schema family, `src/use_case/configuration.go:26,43,60,77,94` the config family. **How to apply:** "I can read the config" proves nothing about writing it — grant `config.update` explicitly before attempting a live edit, and don't use a schema 403 to infer anything about config access. The paragraph below was written from a session where *nothing* was granted, which is why it read as one shared gate:
