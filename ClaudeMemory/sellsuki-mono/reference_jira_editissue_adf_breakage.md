...
**🔴 Thai corruption hits the `description` path but NOT the `commentBody` path — confirmed side by side 2026-09-10:** in one session, three full-description rewrites via `editJiraIssue` + `contentFormat: markdown`: OC-4492 and OC-4370 came back clean, **OC-4491 came back with three silently mangled words** — `เกณฑ์`→`เกกณฑ์`, `ใบสุดท้าย`→`ใบสุดท้า`, `ใด`→`ใหน`. Same session, same tool, same call shape ⇒ **it is non-deterministic, so "it was fine last card" proves nothing.** Every `addCommentToJiraIssue` in the same session kept Thai byte-perfect. **How to apply:** after any description rewrite containing Thai, re-read the returned description and eyeball the Thai (the response already contains it — no extra fetch needed); a mangled word is readable-but-wrong and will never throw. Cheapest repair when the meaning is unchanged: post a short comment listing `wrong = right` rather than resending the whole description (a resend can corrupt different words).


**🟡 bold ที่คร่อม inline-code ถูก ADF ตัดแยก (ยืนยัน 2026-09-10 บน OC-3559):** `**เกณฑ์รักษาระดับ (`retain_points`)**` กลับมาเป็น `**เกณฑ์รักษาระดับ (**`retain_points`)` — เครื่องหมาย bold ไปปิดกลางประโยค อ่านได้แต่เพี้ยน ไม่ error. เกิดกับทุกกรณีที่ `**...`code`...**` · **แก้:** อย่าเอา backtick ไว้ในช่วง bold — ปิด bold ก่อนแล้วต่อ code (`**หัวข้อ** `field``) หรือทิ้ง backtick ในหัวข้อไปเลย.

**🔴 บทเรียนซ้ำรอบเดียวกัน (2026-09-10): การพิมพ์ payload ยาวจากความจำ = แหล่งคำเพี้ยนที่แท้จริง** ครั้งแรกที่ยิง OC-3559 ผมส่งไทยเป็น `\uXXXX` แล้วได้ `เฉพาะ`→`เซพาะ`, `ขึ้น`→`ขื้น`, `คำนวณ`→`คำนวน` — **ไฟล์ต้นฉบับสะอาด** ⇒ ไม่ใช่ ADF ทำ เป็นการถอดความของผมเอง. รอบสองอ่านไฟล์ทั้งไฟล์ด้วย `sed -n` ก่อนแล้วค่อยส่ง → สะอาด. **How to apply:** เนื้อการ์ดยาวให้ compose ในไฟล์ก่อนเสมอ แล้ว **อ่านไฟล์ในเทิร์นเดียวกับที่ส่ง** — อย่าประกอบ payload จากความจำ และเทียบคำหลังส่งด้วย `grep` บนไฟล์ต้นฉบับ ไม่ใช่ตาเปล่า.
